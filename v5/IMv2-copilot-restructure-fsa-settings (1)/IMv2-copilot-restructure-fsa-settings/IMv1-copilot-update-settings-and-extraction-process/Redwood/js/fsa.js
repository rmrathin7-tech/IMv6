...
async function loadFSA() {
    ...
    // Helper: resolve chart of accounts sections for a document definition ID
    function getCoaSections(docDefId) {
      if (coa[fsaDomainId] && Array.isArray(coa[fsaDomainId][docDefId])) return coa[fsaDomainId][docDefId];
      if (coa['shared'] && Array.isArray(coa['shared'][docDefId])) return coa['shared'][docDefId];
      if (Array.isArray(coa[docDefId])) return coa[docDefId];
      for (const key of Object.keys(coa)) {
        if (Array.isArray(coa[key][docDefId])) return coa[key][docDefId];
      }
      return [];
    }
    ...
    configSchemas = {
      ...
      documents: (Array.isArray(domData.documents) ? domData.documents : []).map(docDef => {
        const sections = getCoaSections(docDef.id);
        return {
          id:    docDef.id,
          key:   docDef.key,
          title: docDef.title,
          equityItems: domData.equityItems?.[docDef.id]
                    || entSnap.data()?.equityItems
                    || {},
    ...
    console.log("✅ configSchemas built:", {
      documentCount: configSchemas.documents.length,
      entityTypesKeyed: Object.keys(configSchemas.entityTypes),
      crossDocLinks: configSchemas.crossDocLinks.length
    });
    ...
    // ── ENSURE EQUITY SECTION EXISTS IN BS ────────────────────────────
    const bsDoc = configSchemas.documents.find(d => d.key === 'bs');
    if (bsDoc) {
        const hasEquity = bsDoc.structure.some(s => s.key === 'equity');
        if (!hasEquity) {
            console.log("📝 Adding equity section to BS structure");
...
function renderSection(section) {
    ...
    if (section === "dataEntry") {
        ...
        // Debounce map: path → { originalVal, currentVal, meta }
        ...
        document.getElementById("pdf-import-input").addEventListener("change", async (e) => {
            ...
            try {
                ...
                configSchemas.documents.forEach(docDef => {
                    const sections = docDef.structure.map(s => ({ 
                        key: s.key, 
                        title: s.title,
                        sub_items: s.items || []
                    }));
                ...
                const res = await fetch("https://redwood-api-production.up.railway.app/extract", {
                    method: "POST",
                    body: formData
                });
                ...
                if (result.mapped.company_type) {
                    fsaState.currentFsaData.companyType = result.mapped.company_type;
                    console.log("📊 Company Type Detected:", result.mapped.company_type);
                }
...
window.showImportValidationModal = function(mapped, raw, confidence, periodsToShow = null) {
    ...
    function getConfidenceInfo(itemName) {
        const thresholds = configSchemas.confidenceThresholds || { high: 85, medium: 60 };
        ...
        if (score >= thresholds.high)   return { color: '#10b981', label: 'High', score };
        if (score >= thresholds.medium) return { color: '#f59e0b', label: 'Med',  score };
