...
export function initDataEntry({
    ...
    function buildAnchoredTotals(schema) {
        ...
        (schema?.structure || []).forEach(item => {
            ...
            if (item.type === 'total' && lastSectionKey) {
                if (!anchors[lastSectionKey]) anchors[lastSectionKey] = [];
                anchors[lastSectionKey].push({
                    title: item.title,
                    key:   item.key,
                    color: item.color || '#3b82f6',
                    bg:    item.bg    || 'rgba(59,130,246,0.05)'
                });
                lastSectionKey = null;
            }
        });
    ...
    function renderDataEntry(schema, type) {
       ...
       let html = `
        ...
        (schema?.structure || []).forEach(section => {
            ...
            // Setup BS Dynamic Items
            let dynamicEqTopLevel = null;
            if (section.dynamic && type === "bs") {
