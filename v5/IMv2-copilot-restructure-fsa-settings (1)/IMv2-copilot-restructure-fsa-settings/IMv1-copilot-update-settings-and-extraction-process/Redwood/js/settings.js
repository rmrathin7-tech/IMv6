...
async function loadAll() {
  ...
  // Ensure every document type has an array under 'shared'
  masterDocuments.forEach(d => {
    if (!chartOfAccounts[SHARED_DOMAIN_ID][d.id]) {
...
window.removeDocument      = (i) => {
  const docId = masterDocuments[i].id;
  Object.keys(chartOfAccounts).forEach(dId => { delete chartOfAccounts[dId][docId]; });
...
function handleSchemaDropdownChange() {
  ...
  if (currentDocId) {
    ...
    if (!chartOfAccounts[currentDomainId][currentDocId]) chartOfAccounts[currentDomainId][currentDocId] = [];
  }
  renderSchemaBuilder();
}
